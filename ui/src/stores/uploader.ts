...
export const useUploaderStore = defineStore('uploader', () => {
   ...
   * Uploader的增加上传文件的方法
   */
  let addUploadFileOfUploader = null as unknown as ((file: File, uid: string, path: string | null, uploadedCallback: (uploadPath?: string) => void) => void) | null

  /**
   * 初始化Uploader的增加上传文件的方法
   * @param method
   */
  const initAddUploadFileOfUploader = (method: (file: File, uid: string, path: string | null, uploadedCallback: (uploadPath?: string) => void) => void) => {
   ...
   * 增加上传文件的方法
   */
  const addUploadFile = (file: File, uid: string, path: string | null, uploadedCallback: (uploadPath?: string) => void) => {
