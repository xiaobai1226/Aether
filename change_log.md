...
### feat
1. 去除Redis改为Caffeine本地缓存
2. 增加定时任务删除大于10天的回收站文件
3. 增加markdown预览
4. 图片预览增加文件名显示
5. 拖拽上传文件/文件夹
6. 同目录下图片支持左右切换与数量显示
...
### feat
1. 上传完成列表增加上传完成时间，并改为倒序排列
2. 优化昵称显示逻辑为没有昵称默认显示用户名
3. 优化代码架构

### fix
1. 修复这一页图标是文件夹，进入新的一页图标变成缩略图后，会先展示文件夹图标，加载完缩略图才改为缩略图的问题
2. 导航栏长度过长会隐藏，看不全名称问题
3. 修复登录过期后登陆成功无法自动回到原先的页面问题
4. 代码在mac环境下运行正常，在windows下后端返回给前端的中文乱码
5. 修复头像选择文件时无法选择jpeg格式图片的问题
6. 修复上传文件时应该同时上传三个文件，文件多时可能会变成只同时上传一个
7. 修复上传文件刷新列表，当前目录不是上传文件所属目录不刷新，偶尔会失效
8. 优化新建文件夹时提示文件夹名称为空，显示的是文件/文件夹名称不能为空，要只显示文件夹
9. 修复缩略模式和大图模式，重命名自动选中名字，没有忽略后缀
