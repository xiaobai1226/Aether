...
//    }
        ...
        try {
            ...
            if (this.fileSystem() == null) {
                ...
                try {
                    switch (ctx.method()) {
                        case "OPTIONS":
                            status = this.handleOptions(ctx, userId);
                            break;
                        case "GET":
                        case "HEAD":
                        case "POST":
                        ...
                        case "UNLOCK":
                            ...
                            break;
                        case "PROPFIND":
                            status = this.handlePropfind(ctx, userId);
                            break;
                        case "PROPPATCH":
    ...
    private int handlePropfind(Context ctx, Integer userId) throws Exception {
        ...
        List<FileInfo> childs = this.fileSystem().fileList(reqPath, userId);
    ...
    private String toItemResponse(String reqPath, FileInfo fi) {
        ...
        href = URLUtil.encode(href);
        return StrUtil.format(template, href, DateUtil.parse(fi.update()).toJdkDate().toString(), DateUtil.parse(fi.create()).toString("yyyy-MM-dd'T'HH:mm:ss'Z'"), fi.size(), fi.isDir() ? "<D:collection/>" : "", this.fileSystem().fileMime(fi));
    }
    ...
    private int handlePut(Context ctx, Integer userId) throws Exception {
        ...
        String reqPath = stripPrefix(ctx.path());
        boolean flag = this.fileSystem().putFile(reqPath, ctx.bodyAsStream(), userId);
        return flag ? 204 : 405;
    }
