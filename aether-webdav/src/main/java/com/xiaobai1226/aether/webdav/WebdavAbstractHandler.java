 ...
 */
package com.xiaobai1226.aether.webdav;
...
public abstract class WebdavAbstractHandler implements Handler {
    ...
    private void setCommonHeaders(Context ctx) {
    ...
    private int handleGetHeadPost(Context ctx) throws Exception {
        String reqPath = stripPrefix(ctx.path());
        ...
        if (StrUtil.equals(ctx.method(), "HEAD")) {
            ...
            ctx.contentLength(fileSize);
            return 200;
        }

        String rangeHeader = ctx.header("Range");
        if (this.range && StrUtil.isNotBlank(rangeHeader)) {
            RangeInfo rangeInfo = parseRange(rangeHeader, fileSize);
            if (rangeInfo == null) {
                ctx.headerSet("Content-Range", "bytes */" + fileSize);
                ctx.headerSet("Content-Length", "0");
                return 416;
            }

            long length = rangeInfo.end - rangeInfo.start + 1;
            ctx.status(206);
            ctx.headerSet("Content-Length", String.valueOf(length));
            ctx.headerSet("Content-Range", "bytes " + rangeInfo.start + "-" + rangeInfo.end + "/" + fileSize);
            try (InputStream resIn = this.fileSystem().fileInputStream(reqPath, rangeInfo.start, length)) {
                ctx.output(resIn);
            }
            return 206;
        }
        ...
        ctx.status(200);
        ctx.headerSet("Content-Length", String.valueOf(fileSize));
        try (InputStream resIn = this.fileSystem().fileInputStream(reqPath, 0, 0)) {
            ctx.output(resIn);
        }
        return 200;
    }
    ...
    private RangeInfo parseRange(String rangeHeader, long fileSize) {
        ...
        if (StrUtil.isBlank(spec) || spec.contains(",")) {
            return null;
        }
    ...
    private static class RangeInfo {
        ...
        private RangeInfo(long start, long end) {
            this.start = start;
            this.end = end;
        }
    }
